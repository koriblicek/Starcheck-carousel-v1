...
import { ICarouselImageItem } from 'src/types';
...
export default function SlideItemImage({ slide, itemsPerSlide, isMobile }: ISlideItemProps) {
    ...
    return (
        ...
        >
            ...
            {isAlt &&
                <Box
                    sx={{ color: 'white', position: 'absolute', textAlign: 'left', bottom: 0, width: '100%', backgroundColor: hovered ? "#000000cc" : "#00000022", p: 0, filter: `opacity(${hovered ? .9 : 1})`, cursor: isCta ? 'pointer' : 'auto', transition: 'all .4s' }}
                    onPointerEnter={() => setHovered(true)}
                    onPointerLeave={() => setHovered(false)}
                >
                    <Typography
